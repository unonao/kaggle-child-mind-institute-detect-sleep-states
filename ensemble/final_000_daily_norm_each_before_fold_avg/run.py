...
def daily_normalize_df(
    train_df, name, daily_score_offset, event2offset: dict[str, str] = {"onset": "5h", "wakeup": "0h"}
):
    for event, offset in event2offset.items():
        event_pred_col = f"{name}_stacking_prediction_{event}"
        train_df = (
            train_df.with_columns(pl.col("timestamp").dt.offset_by(offset).dt.date().alias("date"))
...
def cal_score(cfg, name2weight, params, df, event_df):
    ...
    for name in cfg.shimacos_nn_models + cfg.shimacos_models + cfg.sakami_models:
        pred_df = daily_normalize_df(pred_df, name, params[f"{name}_daily_score_offset"])
...
def objective(cfg, trial, names, train_df, event_df):
    ...
    params = {
        f"{name}_daily_score_offset": trial.suggest_float(f"{name}_daily_score_offset", 0, 10, step=0.5)
        for name in names
    }
    params["daily_score_offset"] = trial.suggest_float("daily_score_offset", 0, 20, step=0.5)
...
def run_process(cfg, names, train_df, event_df, study_name):
    ...
    study.optimize(
        lambda trial: objective(cfg, trial, names, train_df, event_df),
