...
def main(cfg: DictConfig):  # type: ignore
    ...
    for fold in range(cfg.n_fold):
        ...
        # valid        
        score, valid_sub_df = cal_score(model_weights, params, valid_df, valid_event_df)

        model_weights_list.append(model_weights)
        params_list.append(params)
        scores.append(score)
        valid_sub_dfs.append(valid_sub_df)
    ...
    oof_sub_df = pl.concat(valid_sub_dfs)
    oof_score = event_detection_ap(
        event_df.to_pandas(),
        oof_sub_df.to_pandas(),
        tqdm_disable=True,
    )
