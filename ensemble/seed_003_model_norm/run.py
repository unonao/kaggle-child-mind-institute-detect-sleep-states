...
def main(cfg: DictConfig):  # type: ignore
    ...
    for name in model_names:
        pred_df = pred_df.with_columns(
            pl.col(f"{name}_stacking_prediction_onset") / pl.col(f"{name}_stacking_prediction_onset").max()
        ).with_columns(
            pl.col(f"{name}_stacking_prediction_wakeup") / pl.col(f"{name}_stacking_prediction_wakeup").max()
        )
