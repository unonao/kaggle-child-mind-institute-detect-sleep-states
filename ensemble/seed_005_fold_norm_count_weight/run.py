...
def cal_score(name2weight, params, pred_df, event_df):
    pred_df = pred_df.with_columns(
        ...
        # 数が少ないところは補正する
        pl.col("stacking_prediction_onset") * len(name2weight) / pl.col("count"),
        pl.col("stacking_prediction_wakeup") * len(name2weight) / pl.col("count"),
    )
...
def objective(trial, names, train_df, event_df):
    ...
    score, _ = cal_score(dict(zip(names, weights)), params, train_df, event_df)
    return score
...
def main(cfg: DictConfig):  # type: ignore
    ...
    for fold in range(5):
        ...
        pred_df = pred_df.with_columns(
            pl.when(pl.col("series_id").is_in(valid_ids)).then(pl.lit(fold)).otherwise(pl.col("fold"))
        )

    # norm
    for name in model_names:
        pred_df = pred_df.with_columns(
            pl.col(f"{name}_stacking_prediction_onset") / pl.col(f"{name}_stacking_prediction_onset").max().over('fold')
        ).with_columns(
            pl.col(f"{name}_stacking_prediction_wakeup") / pl.col(f"{name}_stacking_prediction_wakeup").max().over('fold')
        )
    ...
    for seed in range(cfg.seed, cfg.seed+cfg.n_seed):
        ...
        seed_model_weights_list.append(mean_best_model_weights)
